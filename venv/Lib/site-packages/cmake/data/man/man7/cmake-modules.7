...
.
.TH "CMAKE-MODULES" "7" "May 27, 2021" "3.20.3" "CMake"
.SH NAME
...
\fBCPACK_THREADS\fP can be set to positive integer to specify how many threads
...
.sp
Currently only \fBxz\fP compression \fImay\fP take advantage of multiple cores. Other
...
build (if possible) so that the main build can use the populated project\(aqs
...
populate some content in CMake\(aqs script mode.
.sp
Changed in version 3.14: \fBFetchContent\fP commands can access the terminal. This is necessary
...
\fB<lcName>_POPULATED\fP variable is set in the caller\(aqs scope with this form.
.sp
...
.sp
New in version 3.18: Added support for \fBDOWNLOAD_NO_EXTRACT\fP and \fBSOURCE_SUBDIR\fP options.
...
.sp
New in version 3.4: Support for response files (prefixed by \fB@\fP) in the \fBSOURCES\fP list.

.sp
The default \fBOUTPUT_DIR\fP can also be changed by setting the variable
\fBCMAKE_JAVA_TARGET_OUTPUT_DIR\fP\&.
.sp
New in version 3.11: Optionally, using option \fBGENERATE_NATIVE_HEADERS\fP, native header files can
...
.sp
New in version 3.20: \fBDESTINATION\fP sub\-option now supports the possibility to specify different
output directories for \fBBUILD\fP and \fBINSTALL\fP steps. This is required to
export the interface target generated by \fBGENERATE_NATIVE_HEADERS\fP option.
If \fBBUILD\fP directory is not specified, a default directory will be used.
...
.UNINDENT
.sp
New in version 3.20: The target generated by option \fBGENERATE_NATIVE_HEADERS\fP has the property
\fBNATIVE_HEADERS_DIRECTORY\fP which specify the directory holding the native
headers.
...
.sp
New in version 3.9: The \fBNAMESPACE\fP option.
...
.UNINDENT
.sp
New in version 3.20: It is now possible to export the target generated by
\fBGENERATE_NATIVE_HEADERS\fP option.
.INDENT 0.0
.INDENT 3.5
.sp
.nf
...
and \fBVERSION\fP\&.
.sp
If you don\(aqt set the \fBINSTALLPATH\fP, then by default the documentation will
be installed to :
.INDENT 0.0
.INDENT 3.5
.sp
...
Makefile and
Ninja generators. Default value is \fBFALSE\fP\&.
.TP
...
.UNINDENT
.sp
New in version 3.6: \fBOpenBLAS\fP support.

.sp
New in version 3.11: \fBFLAME\fP support.

.sp
New in version 3.13: Added ILP64 MKL variants (\fBIntel10_64ilp\fP, \fBIntel10_64ilp_seq\fP).

.sp
New in version 3.17: Added single dynamic library MKL variant (\fBIntel10_64_dyn\fP).

.sp
New in version 3.18: Arm Performance Libraries support (\fBArm\fP, \fBArm_mp\fP, \fBArm_ilp64\fP,
\fBArm_ilp64_mp\fP).

.sp
New in version 3.19: \fBFlexiBLAS\fP support.

.sp
New in version 3.20: Elbrus Math Library support (\fBEML\fP, \fBEML_mt\fP).
...
compiler wrapper as \fBCMAKE_<LANG>_COMPILER\fP or if they\(aqre on a Cray system.
.IP 3. 3
...
internally.
.sp
New in version 3.1: The \fBPKG_CONFIG\fP environment variable can be used as a hint.
...
.sp
New in version 3.19: This module defines \fBIMPORTED\fP target \fBVulkan::glslc\fP, if
...
(As of CMake version 3.20.3 there is only one API version.)
.sp
...
.sp
New in version 3.17: To find and use the CUDA toolkit libraries the \fBFindCUDAToolkit\fP
