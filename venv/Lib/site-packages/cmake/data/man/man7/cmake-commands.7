...
.
.TH "CMAKE-COMMANDS" "7" "May 27, 2021" "3.20.3" "CMake"
.SH NAME
...
.TE
.sp
New in version 3.10: Additional \fB<key>\fP values are available:
...
\fI\%Native Conversion\fP
  ...
  cmake_path(\fI\%CONVERT\fP <input> \fI\%TO_CMAKE_PATH_LIST\fP <out\-var>)
  cmake_path(\fI\%CONVERT\fP <input> \fI\%TO_NATIVE_PATH_LIST\fP <out\-var>)
...
that library\(aqs own dependencies. Otherwise, resolution proceeds to step 6.
.IP 6. 3
...
recursively resolves that library\(aqs own dependencies.
.UNINDENT
.sp
...
.UNINDENT
.sp
New in version 3.16: Added the \fBCMAKE_FIND_USE_<CATEGORY>_PATH\fP variables to globally disable
various search locations.
...
.sp
Changed in version 3.19: This command been superseded by \fBcmake_path()\fP command, except
...
assign the last element\(aqs value to the given variable and removes it,
...
.sp
If no variable name is given, removes exactly one element. Otherwise,
assign the first element\(aqs value to the given variable and removes it,
up to the last variable name given.
.INDENT 0.0
.INDENT 3.5
.sp
...
.UNINDENT
.sp
New in version 3.6: \fB%s\fP format specifier (UNIX time).

.sp
New in version 3.7: \fB%a\fP and \fB%b\fP format specifiers (abbreviated month and weekday names).

.sp
New in version 3.8: \fB%%\fP specifier (literal \fB%\fP).

.sp
New in version 3.7: \fB%A\fP and \fB%B\fP format specifiers (full month and weekday names).
...
.sp
Specify a \fB\&.d\fP depfile for the \fBNinja\fP generator and
Makefile Generators\&.
A \fB\&.d\fP file holds dependencies usually emitted by the custom
command itself.
Using \fBDEPFILE\fP with other generators than \fBNinja\fP or
Makefile Generators is an error.
.sp
New in version 3.20: Added the support of Makefile Generators\&.
...
(see policy \fBCMP0116\fP\&. This policy is always \fBNEW\fP for
Makefile Generators).
.sp
\fBNOTE:\fP
.INDENT 7.0
.INDENT 3.5
For Makefile Generators, this option cannot be specified at the
same time as \fBIMPLICIT_DEPENDS\fP option.
.UNINDENT
.UNINDENT
.UNINDENT
