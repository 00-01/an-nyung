...
.
.TH "CMAKE-PROPERTIES" "7" "May 27, 2021" "3.20.3" "CMake"
.SH NAME
...
known at configure time and therefore can\(aqt be passed to
\fBsource_group()\fP\&.
...
.sp
Supported values are \fB98\fP, \fB03\fP, \fB11\fP, \fB14\fP, \fB17\fP, \fB20\fP, \fB23\fP\&.
.sp
If the value requested does not result in a compile flag being added for
the compiler in use, a previous standard flag will be added instead.  This
means that using:
...
.sp
Supported values are \fB98\fP, \fB11\fP, \fB14\fP, \fB17\fP, \fB20\fP, \fB23\fP\&.
.sp
If the value requested does not result in a compile flag being added for
the compiler in use, a previous standard flag will be added instead.  This
means that using:
...
as well.  The \fBCMAKE_CFG_INTDIR\fP variable can be a convenient
...
.UNINDENT
.sp
When any Xcode variable or \fBCMAKE_CFG_INTDIR\fP is used, CMake is
not able to fully evaluate the path(s) at configure time.  One consequence
of this is that the configuration\-specific
\fBIMPORTED_OBJECTS_<CONFIG>\fP properties cannot be used, since
CMake cannot determine whether an object file exists at a particular
\fB<CONFIG>\fP location.  The \fBIMPORTED_OBJECTS\fP property must be used for
these situations and the configuration\-specific aspects of the path must be
handled by using \fBCMAKE_CFG_INTDIR\fP or with another Xcode variable
\fB$(CONFIGURATION)\fP\&.
...
\fBFortran\fP, \fBISPC\fP, \fBOBJC\fP, \fBOBJCXX\fP, or \fBCUDA\fP\&.
.sp
...
3.20.3 supports combining \fBC\fP and \fBCXX\fP source files.  For targets that
...
.sp
Currently, the only supported value for \fB<type>\fP is \fBFRAMEWORKS\fP\&.
...
\fBXCODE_EMBED_FRAMEWORKS_REMOVE_HEADERS_ON_COPY\fP and
\fBXCODE_EMBED_FRAMEWORKS_CODE_SIGN_ON_COPY\fP\&.
.SS XCODE_EMBED_<type>_PATH
.sp
New in version 3.20.

.sp
Tell the \fBXcode\fP generator the relative path to use when embedding
the items specified by \fBXCODE_EMBED_<type>\fP\&.  The path is relative
...
\fBCSharp\fP, \fBCUDA\fP, \fBFortran\fP, \fBISPC\fP, and \fBASM\fP\&.  Setting this
